...
class SX1262(SX126X):
    ...
    def begin(self, freq=434.0, bw=125.0, sf=9, cr=7, syncWord=SX126X_SYNC_WORD_PRIVATE,
              ...
              crcOn=True, txIq=False, rxIq=False, txPa=False, tcxoVoltage=1.6, useRegulatorLDO=False,
              blocking=True):
        state = super().begin(bw, sf, cr, syncWord, currentLimit, preambleLength, tcxoVoltage, useRegulatorLDO, txIq, rxIq, txPa)
        ASSERT(state)
    ...
    def setOutputPower(self, power):
        if not ((power >= -9) and (power <= (27 if self._txPa else 22))):
